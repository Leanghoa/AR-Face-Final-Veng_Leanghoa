...
public sealed class EditorArFaceSchoolDemo : MonoBehaviour
{
...
#if UNITY_EDITOR
    [Header("Auto texture import (Editor only)")]
    [SerializeField]
    bool autoIncludeAllTextures = false;

    [SerializeField]
    string texturesSearchFolder = "Assets";
    ...
    // Current runtime list used for cycling (includes generated filters when enabled)
    GameObject[] _currentFilterList;

#if UNITY_EDITOR
    readonly List<GameObject> _generatedFilters = new List<GameObject>();
#endif
...
#if UNITY_EDITOR
        foreach (var g in _generatedFilters)
        {
            if (g != null)
                DestroyImmediate(g);
        }
        _generatedFilters.Clear();
#endif
    }
    ...
    IEnumerator EditorDemoSetup()
    {
        ...
        if (autoIncludeAllTextures)
        {
            ...
            foreach (string g in guids)
            {
                ...
                go.name = "GenFilter_" + tex.name;
                var mr = go.GetComponent<MeshRenderer>();
                var mat = new Material(Shader.Find("Unlit/Texture"));
                mat.mainTexture = tex;
                mr.sharedMaterial = mat;
                _generatedFilters.Add(go);
                merged.Add(go);
                added++;
            }
        }
        ...
        _currentFilterList = merged.ToArray();
        ...
        _filterIndex = 0;
        SpawnActiveFilterPrefab(_currentFilterList);

        if (autoCreateFilterSwitchUi && _currentFilterList.Length > 1)
    ...
    public void CycleToNextFilter()
    {
        ...
        GameObject[] list = _currentFilterList != null ? _currentFilterList : EffectiveFilterPrefabs();
